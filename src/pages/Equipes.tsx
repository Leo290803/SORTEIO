...
const nomeModalidadePeloSlug = (slug?: string) => {
  if (!slug) return "";

  const encontrada = MODALIDADES.find((m) => slugifyModalidade(m) === slug);
...
const Equipes = () => {
  ...
  const modalidadeSelecionada = useMemo(
    () => nomeModalidadePeloSlug(modalidade),
    [modalidade]
  );
  ...
  const [form, setForm] = useState({
    ...
    modalidade: modalidadeSelecionada || (MODALIDADES[0] as string),
    atleta1: "",
    atleta2: "",
    escola: "",
  });
  ...
  const formInfo = useMemo(
    () => getModalidadeInfo(form.modalidade),
    [form.modalidade]
  );
  ...
  const openNew = () => {
    ...
    setForm({
      ...
      naipe: NAIPES[0],
      modalidade: modalidadeSelecionada || MODALIDADES[0],
      atleta1: "",
      atleta2: "",
      escola: "",
    });
  ...
  const save = async () => {
    ...
    const base = {
      ...
      naipe: form.naipe,
      modalidade: modalidadeSelecionada || form.modalidade,
    };
    ...
    if (info.tipo === "atletas") {
      ...
      if (info.atletas === 2 && !a2) {
        return toast.error("Informe os dois atletas da dupla");
      }
      ...
      if (!form.nome.trim()) {
        return toast.error("Informe o nome da equipe / escola");
      }
  ...
  const filtered = equipes.filter((e) => {
    if (modalidade && slugifyModalidade(e.modalidade) !== modalidade) {
      return false;
    }
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="container py-8 md:py-12">
        <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight mt-2">
              {modalidadeSelecionada || "Cadastros"}
            </h1>

            <p className="text-muted-foreground mt-1">
              Cadastros separados por modalidade.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 p-4 rounded-xl bg-card/60 border border-border mb-6">
          <div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 rounded-xl border border-dashed border-border bg-card/30">
            ...
            <p className="text-muted-foreground">
              Nenhum cadastro encontrado nessa modalidade.
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className={
                        e.ativo
                          ? ""
                          : "border-winner/40 text-winner hover:bg-winner/10"
                      }
                    >
                      {e.ativo ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select
                  value={form.naipe}
                  onValueChange={(v) => setForm({ ...form, naipe: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            {formInfo.tipo === "escola" ? (
              ...
              <div className="space-y-3 p-4 rounded-lg bg-secondary/40 border border-border">
                ...
                <div>
                  <Label>
                    {formInfo.atletas === 2 ? "Atleta 1" : "Nome do atleta"}
                  </Label>

                  <Input
                    value={form.atleta1}
                    onChange={(ev) =>
                      setForm({ ...form, atleta1: ev.target.value })
                    }
                ...
                {formInfo.atletas === 2 && (
                  <div>
                    ...
                    <Input
                      value={form.atleta2}
                      onChange={(ev) =>
                        setForm({ ...form, atleta2: ev.target.value })
                      }
                      placeholder="Nome completo"
                    />
                  </div>
                ...
                <div>
                  ...
                  <Input
                    value={form.escola}
                    onChange={(ev) =>
                      setForm({ ...form, escola: ev.target.value })
                    }
